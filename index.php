...
$searchSummoners = $_GET['searchSummoners'] ?? '';
...
//    var_dump(getSummonerSpellInfo(1));
    ...
    <article>
        ...
        <div class="searching_area">
            <form action="#" method="get" style="width: 100%; display: flex">
                <input class="" type="search" name="searchSummoners" placeholder="Enter summoner name"
                       value=""
                       placeholder="Nom d'utilisateur" autofocus=""/>
                <button class="button_summoner" type="submit" name="operation" value="search">Run</button>
            </form>
        </div>
    </article>
</div>
<?php if ($operation == 'search') { ?>
    <?php if ($rep == "good") { ?>
                            ...
                            <div class="collum_red">
                                ...
                                <?php for ($i = 5; $i < 10; $i++) {
                                    ...
                                    <div class="champ_row">
                                        ...
                                        <div class="summ_name">
                                            <div><a style="color: black;"
                                                    href="?searchSummoners=<?php echo $datasRecherchePartie['participants'][$i]['summonerName'] ?>&operation=search#"><?php echo $datasRecherchePartie['participants'][$i]['summonerName'] ?></a>
                                            </div>
                                ...
                                foreach ($matchLolByGames['info']['participants'] as $arrInfoParticipants) {
                                    if ($arrInfoParticipants['summonerName'] == $datasSummoners['name']) {
                                        ...
                                        <div class=collum_size_champ>
                                            <a href="/champion/champion_page.php?operation=search&nomChamp=<?= $nomChampionGame['id'] ?>"><img
    ...
    <?php
    if ($messagePseudoInconnu != 'Utilisateur non trouvé')
...
for($i = 0 ; $i < 3 ; $i++)
{
    ...
    $masteries = $datasChampMasteriesSummoner[$i]['championLevel'];
    $pointMasteries = $datasChampMasteriesSummoner[$i]['championPoints'];
    $imagePerso = 'http://ddragon.leagueoflegends.com/cdn/'.$latest.'/img/champion/'.$champInfo['name'].'.png';
    ?>
        <h3><?php echo $champInfo['name'] ?></h3>
        <h3><?php echo $masteries ?></h3>
        <h3><?php echo $pointMasteries ?></h3>
    <img src="<?php echo $imagePerso ?>" alt="PhotoChamp">
<?php
}
$matchLol = $europeApi . '/lol/match/v5/matches/by-puuid/'.$puuid.'/ids';
$idMatchLol = $Api->requestApi($matchLol, 'GET', '', '');
//var_dump($idMatchLol);
...
<?php
    $matchLolById= $europeApi . '/lol/match/v5/matches/'.$arrMatch;
    $MatchLolByGames = $Api->requestApi($matchLolById, 'GET', '', '');
//    var_dump($MatchLolByGames['info']['participants']);
?>
...
//            var_dump($i);
            ?>
<!---->
<!--                    <h3>--><?php //echo $MatchLolByGames['info']['participants'][$i]['summonerName']?><!-- </h3>-->
<!--                    <h3>--><?php //echo $MatchLolByGames['info']['participants'][$i]['championName'] ?><!-- </h3>-->
<!--                    <h3>--><?php //echo $MatchLolByGames['info']['participants'][$i]['champLevel'] ?><!-- </h3>-->
...
//        $summonersByPuuid = $europeApi . '/riot/account/v1/accounts/by-puuid/' . $arrSummoners;
//        $detailSummonersByPuuid = $Api->requestApi($summonersByPuuid, 'GET', '', '');
//        var_dump($detailSummonersByPuuid['gameName']);
